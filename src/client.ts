 ...
 *
 * 源码保持纯 ESM；构建（tsdown，tsdown.client.config.mjs）把产出的 CJS 包进
 * web profile 的 `__ModuleLoader__.load({ id, factory })` 壳。ESM 模块作用域天然
 * 隔离共享 window 全局——无需 IIFE，顶层声明也不会与兄弟 bundle 撞名。
 */
 ...
 * 别名，存在竞态。该模块在产物中保持 external，经构建壳的 `require` 取共享实例。
 */
import { Button, Card, Checkbox, Col, Field, MenuSelect, Row, Spinner, TextInput, Textarea, T } from '@dsh-plugin/dsh-loader/client';
...
/** cordis client-plugin 名——必须等于包名。 */
export const name = '@dsh-plugin/dsh-network-settings';

/** 浏览器半消费的 cordis 服务。 */
export const inject = ['slots'];

export function apply(ctx: { get<T = unknown>(service: string): T | undefined }): void {
  const slots = ctx.get<{
