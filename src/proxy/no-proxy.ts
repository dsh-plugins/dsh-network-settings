/**
 * NO_PROXY 判定（纯函数，移植自 dsh-net-proxy，TS 化）。
 */
 ...
 */
export function isNoProxy(rawUrl: string, noProxy?: string[] | null): boolean {
  ...
  for (const raw of noProxy) {
    ...
    // 点前缀形式（`.example.com`）与裸 host 均按子域/精确匹配
    const dotted = onlyHost.startsWith('.') ? onlyHost : '.' + onlyHost;
    if (host === onlyHost || host.endsWith(dotted)) return true;
  }
  return false;
}